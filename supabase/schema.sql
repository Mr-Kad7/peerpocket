...
create table if not exists learning_submissions(id uuid primary key default gen_random_uuid(),full_name text not null,phone text not null,skill text not null,location text,application_data jsonb not null default '{}'::jsonb,status text not null default 'review' check(status in('review','approved','rejected')),admin_notes text,created_at timestamptz not null default now());
alter table pitch_submissions add column if not exists application_data jsonb not null default '{}'::jsonb;
create table if not exists supporters(id uuid primary key default gen_random_uuid(),display_name text,role text,phone_last4 text,total_supported numeric(12,2) not null default 0,campaigns_supported integer not null default 0,created_at timestamptz not null default now());
alter table supporters add column if not exists role text;
...
alter table campaigns enable row level security; alter table pitch_submissions enable row level security; alter table learning_submissions enable row level security; alter table supporters enable row level security; alter table transactions enable row level security; alter table campaign_updates enable row level security; alter table reports enable row level security; alter table site_settings enable row level security;
