...
create table if not exists pitch_submissions(id uuid primary key default gen_random_uuid(),title text not null,entrepreneur_name text not null,category text not null,location text,goal numeric(12,2) not null,story text not null,use_of_funds text not null,image_url text,status text not null default 'review' check(status in('review','approved','rejected')),admin_notes text,created_at timestamptz not null default now());
